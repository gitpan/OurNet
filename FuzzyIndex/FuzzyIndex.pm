...
sub new {
    ...
    $self->{'submod'}   = ($self->{'submin'} or $self->{'submax'} >= $self->{'subcount'})
                              ? $self->{'subcount'}
                              : 0;
    ...
    foreach my $num ($self->{'submin'}..$self->{'submax'}) {
        $self->{'subobj'}[$num] = tie (%{$self->{'subdb'}[$num]},
                                       'DB_File',
                                       "$self->{'dbfile'}.$num",
                                       $self->{'flag'},
                                       $self->{'flag'} ? 0640 : 0440,
    ...
    if (not ref($_[0])) {
        ...
        if (!$self->{'submod'}) {
            ...
            while (my ($entry, $freq) = each %{$matchref}) {
                ...
                if ($v = substr($entry, -2) eq "  ") {
                    # Latin-1
                    $self->{'subdb'}[$thismod]{substr($entry, 0, -2)} = $id."  ".chr($freq);
                }
                elsif (($k = substr($entry, 0, -2)) eq $lastkey) {
                    # Big-5 continued
                    $lastval .= $v.chr($freq);
